...
const ManualEntryForm = ({ onCancel, initialData, onSubmissionSuccess }) => {
 ...
 useEffect(() => {
  if (initialData) {
   ...
   setReceivedDate(formatDateForInput(new Date().toISOString()));
 ...
 const handleSubmit = async (e) => {
  ...
  try {
   ...
   const errorMessage = error.payload || error.message || error.toString() || 'Unknown error'; // Prefer payload if available
