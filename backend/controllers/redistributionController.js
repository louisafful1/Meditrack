...
// @desc    Get AI-based redistribution suggestions
// @route   GET /api/redistribution/suggestions
// @access  Private
export const getRedistributionSuggestions = asyncHandler(async (req, res) => {
  const inventory = await Inventory.find().populate('facility');

  // Group by drugName + batchNumber
  const drugMap = {};

  inventory.forEach(item => {
    const key = `${item.drugName}|${item.batchNumber}`;
    if (!drugMap[key]) drugMap[key] = [];
    drugMap[key].push(item);
  });

  const suggestions = [];

  for (const [key, items] of Object.entries(drugMap)) {
    const [drugName, batchNumber] = key.split('|');

    // Sort items by current stock
    items.sort((a, b) => b.currentStock - a.currentStock);

    const sender = items[0];
    const receiver = items[items.length - 1];

    // Suggest only if there's imbalance
    if (sender.currentStock > 50 && receiver.currentStock < 10 && sender.facility._id.toString() !== receiver.facility._id.toString()) {
      const expiryDate = new Date(sender.expiryDate);
      ...
      suggestions.push({
        drugName,
        batchNumber,
        quantity: Math.min(20, sender.currentStock),
        fromFacility: {
          id: sender.facility._id,
          name: sender.facility.name
        },
        toFacility: {
          id: receiver.facility._id,
          name: receiver.facility.name
        },
        reason: daysToExpiry < 30
          ? 'Stock expiring soon, and receiving facility has low supply'
          : 'Stock imbalance detected',
        expiryDate: sender.expiryDate,
        daysToExpiry
      });
    }
  }
